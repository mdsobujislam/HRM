...
namespace HRM.Services
{
    public class EmployeeService : IEmployeeService
    {
            ...
            try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        {
                            var subscriptionId = _baseService.GetSubscriptionId();
                            var userId = _baseService.GetUserId();   // string
                            var branchId = await _baseService.GetBranchId(subscriptionId, userId);
                            ...
                            // Insert Employee
                            var employeeInsertQuery = @"
                        INSERT INTO Employees
                        (EmpId, EmployeeName, FatherName, MotherName, SpouseName, DOB, Gender,
                         BloodGroup, MaritalStatus, TinNo, NationalID, Telephone, MobileNo, MailID,
                         PermanentAddress, PresentAddress, DateOfAppointment, Religion, Nationality,
                         Shift, Version, JobType, UploadPhoto, BranchId, DepartmentId, DesignationId,
                         SubscriptionId, CompanyId, CreatedAt, AdditionalId, Status)
                        VALUES
                        (@EmpId, @EmployeeName, @FatherName, @MotherName, @SpouseName, @DOB, @Gender,
                         @BloodGroup, @MaritalStatus, @TinNo, @NationalID, @Telephone, @MobileNo, @MailID,
                         @PermanentAddress, @PresentAddress, @DateOfAppointment, @Religion, @Nationality,
                    ...
                    ";
                            ...
                            var employeePkId = await connection.ExecuteScalarAsync<int>(
                                employeeInsertQuery,
                                new
                                {
                            ...
                            var userInsertQuery = @"
                        INSERT INTO Users
                        (Name, MobileNo, Email, Password, SubscriptionId, BranchId, Status, CreatedAt, EmployeeId)
                        VALUES
                        (@Name, @MobileNo, @Email, @Password, @SubscriptionId, @BranchId, 1, @CreatedAt, @EmployeeId)
                    ";
                            ...
                            await connection.ExecuteAsync(
                                ...
                                new
                                {
                                    ...
                                    Password = 123456,    // default password = empId
                                    SubscriptionId = subscriptionId,
                                    BranchId = branchId,
                                    CreatedAt = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
                            ...
                            // Commit transaction only if both inserts succeed
                            transaction.Commit();
                            return true;
                        }
                        catch
        ...
        //            var queryString = "insert into Employees (EmpId,EmployeeName,FatherName,MotherName,SpouseName,DOB,Gender,BloodGroup,MaritalStatus,TinNo,NationalID,Telephone,MobileNo,MailID,PermanentAddress,PresentAddress,DateOfAppointment,Religion,Nationality,Shift,Version,JobType,UploadPhoto,BranchId,DepartmentId,DesignationId,SubscriptionId,CompanyId,CreatedAt,AdditionalId,Status) values ";
        //            queryString += "( @EmpId,@EmployeeName,@FatherName,@MotherName,@SpouseName,@DOB,@Gender,@BloodGroup,@MaritalStatus,@TinNo,@NationalID,@Telephone,@MobileNo,@MailID,@PermanentAddress,@PresentAddress,@DateOfAppointment,@Religion,@Nationality,@Shift,@Version,@JobType,@UploadPhoto,@BranchId,@DepartmentId,@DesignationId,@SubscriptionId,@CompanyId,@CreatedAt,@AdditionalId,@Status)";
        //            var parameters = new DynamicParameters();
