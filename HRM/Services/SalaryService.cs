        ...
        public async Task<bool> InsertSalary(Salary salary)
        {
            try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    ...
                    var queryString = "insert into OffDays (MonthIndex,Month,Year,FromDate,ToDate,GenDate,EmployeeId,DesignationId,DepartmentId,BranchId,Sl,Parameter,Value,FinalAmount,TxId,RefTxId,SubscriptionId,CompanyId,CreatedAt) values ";
                    queryString += "( @MonthIndex,@Month,@Year,@FromDate,@ToDate,@GenDate,@EmployeeId,@DesignationId,@DepartmentId,@BranchId,@Sl,@Parameter,@Value,@FinalAmount,@TxId,@RefTxId,@SubscriptionId,@CompanyId,@CreatedAt)";
                    var parameters = new DynamicParameters();
                    //parameters.Add("OffDay", salary.OffDay, DbType.String);
